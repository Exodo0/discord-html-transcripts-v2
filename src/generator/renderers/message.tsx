...
import { MessageFlags, type Message as MessageType } from 'discord.js';
...
export default async function DiscordMessage({
  ...
  return (
    ...
    >
      ...
      {/* message content - only for non-V2 messages */}
      {!isComponentsV2 && message.content && (
        <MessageContent
      ...
      {/* attachments - only for non-V2 messages */}
      {!isComponentsV2 && <Attachments message={message} context={context} />}

      {/* message embeds - only for non-V2 messages */}
      {!isComponentsV2 &&
