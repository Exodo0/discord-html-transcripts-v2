...
export function DiscordButton({ type, url, emoji, children, disabled = false }: DiscordButtonProps) {
  ...
  // emoji can be a URL (custom emoji) or a unicode string
  const isUrl = emoji && (emoji.startsWith('http') || emoji.startsWith('//'));

  const emojiNode = emoji ? (
    isUrl ? (
      <img src={emoji} alt="emoji" style={{ width: '16px', height: '16px', marginRight: children ? '6px' : '0', verticalAlign: 'middle', flexShrink: 0 }} />
    ) : (
      <span style={{ marginRight: children ? '6px' : '0', fontSize: '16px', lineHeight: 1, flexShrink: 0 }}>{emoji}</span>
    )
  ) : null;
