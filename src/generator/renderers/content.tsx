...
export async function MessageSingleASTNode({ node, context }: { node: SingleASTNode; context: RenderContentContext }) {
  ...
  switch (type) {
    ...
    case 'emoticon':
      return renderUnknownContent(node.content, context);
...
function renderUnknownContent(content: unknown, context: RenderContentContext): React.ReactNode {
  if (typeof content === 'string') return content;
  if (Array.isArray(content)) return <MessageASTNodes nodes={content as ASTNode} context={context} />;
...
function isLegacyElementLike(
  ...
  return 'type' in value && 'props' in value && '$$typeof' in value;
}
